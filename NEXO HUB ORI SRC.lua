...
local ConfigFileName = "NEXO_Config.json"
...
coordESPFolder.Name = "Nexo_CoordESP"
...
local function executeSteal(prompt, name)
    ...
    progressConnection = RunService.Heartbeat:Connect(function()
        ...
        if ProgressPercentLabel then 
            ...
            ProgressPercentLabel.Text = getProgressText(percent)
        end
    end)
    task.spawn(function()
...
-- ============================================
-- GUI - NEXO HUB - UKURAN SEDANG + TEKS BESAR
-- ============================================
local isMobile = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled
local guiScale = isMobile and 0.5 or 1.2  -- Scale 1.2 agar teks lebih besar
...
local C = {
    ...
    textDim = Color3.fromRGB(180, 200, 255),  -- Dibikin lebih terang
...
progressBar.Size = UDim2.new(0, 500 * guiScale, 0, 70 * guiScale)  -- Lebih besar
progressBar.Position = UDim2.new(0.5, -250 * guiScale, 1, -200 * guiScale)
progressBar.BackgroundColor3 = Color3.fromRGB(2, 2, 4)
progressBar.BorderSizePixel = 0
progressBar.ClipsDescendants = true
Instance.new("UICorner", progressBar).CornerRadius = UDim.new(0, 18 * guiScale)

local pStroke = Instance.new("UIStroke", progressBar)
pStroke.Thickness = 3  -- Lebih tebal
...
for i = 1, 15 do
    ...
    ball.Size = UDim2.new(0, math.random(3, 5), 0, math.random(3, 5))  -- Lebih besar
...
ProgressLabel.Position = UDim2.new(0, 15 * guiScale, 0, 0)
ProgressLabel.BackgroundTransparency = 1
ProgressLabel.Text = "READY"
ProgressLabel.TextColor3 = C.text
ProgressLabel.Font = Enum.Font.GothamBold
ProgressLabel.TextSize = 18 * guiScale  -- Lebih besar
...
ProgressPercentLabel.TextSize = 22 * guiScale  -- Lebih besar
...
RadiusInput.Size = UDim2.new(0, 50 * guiScale, 0, 28 * guiScale)  -- Lebih besar
RadiusInput.Position = UDim2.new(1, -60 * guiScale, 0, 2 * guiScale)
...
RadiusInput.TextSize = 14 * guiScale  -- Lebih besar
RadiusInput.ZIndex = 3
Instance.new("UICorner", RadiusInput).CornerRadius = UDim.new(0, 8 * guiScale)
...
pTrack.Size = UDim2.new(0.94, 0, 0, 10 * guiScale)  -- Lebih tebal
pTrack.Position = UDim2.new(0.03, 0, 1, -20 * guiScale)
...
-- Main Window - Ukuran sedang (560x740) tapi teks lebih besar
local main = Instance.new("Frame", sg)
main.Name = "Main"
main.Size = UDim2.new(0, 560 * guiScale, 0, 740 * guiScale)
main.Position = isMobile and UDim2.new(0.5, -280 * guiScale, 0.5, -370 * guiScale) or UDim2.new(1, -600, 0, 30)
main.BackgroundColor3 = Color3.fromRGB(2, 2, 4)
main.BorderSizePixel = 0
main.Active = true
main.Draggable = true
main.ClipsDescendants = true
Instance.new("UICorner", main).CornerRadius = UDim.new(0, 22 * guiScale)  -- Lebih besar

local mainStroke = Instance.new("UIStroke", main)
mainStroke.Thickness = 3  -- Lebih tebal
...
for i = 1, 60 do
    ...
    ball.Size = UDim2.new(0, math.random(3, 6), 0, math.random(3, 6))  -- Lebih besar
...
header.Size = UDim2.new(1, 0, 0, 80 * guiScale)  -- Lebih tinggi
...
titleLabel.Position = UDim2.new(0, 0, 0, 15 * guiScale)
titleLabel.BackgroundTransparency = 1
titleLabel.Text = "NEXO HUB"
titleLabel.TextColor3 = C.text
titleLabel.Font = Enum.Font.GothamBlack
titleLabel.TextSize = 34 * guiScale  -- Lebih besar
titleLabel.TextXAlignment = Enum.TextXAlignment.Center
titleLabel.ZIndex = 5

local subtitleLabel = Instance.new("TextLabel", header)
subtitleLabel.Size = UDim2.new(1, 0, 0, 28 * guiScale)
subtitleLabel.Position = UDim2.new(0, 0, 0, 50 * guiScale)
subtitleLabel.BackgroundTransparency = 1
subtitleLabel.Text = ""  -- DIHILANGKAN
subtitleLabel.TextColor3 = C.purpleLight
subtitleLabel.Font = Enum.Font.GothamBold
subtitleLabel.TextSize = 20 * guiScale
subtitleLabel.TextXAlignment = Enum.TextXAlignment.Center
subtitleLabel.ZIndex = 5
subtitleLabel.Visible = false  -- Disembunyikan

local closeBtn = Instance.new("TextButton", header)
closeBtn.Size = UDim2.new(0, 44 * guiScale, 0, 44 * guiScale)  -- Lebih besar
closeBtn.Position = UDim2.new(1, -54 * guiScale, 0.5, -22 * guiScale)
closeBtn.BackgroundTransparency = 1
closeBtn.Text = "×"
closeBtn.TextColor3 = C.textDim
closeBtn.Font = Enum.Font.GothamBold
closeBtn.TextSize = 32 * guiScale  -- Lebih besar
closeBtn.ZIndex = 5
...
leftSide.Position = UDim2.new(0.01, 0, 0, 85 * guiScale)
...
rightSide.Position = UDim2.new(0.51, 0, 0, 85 * guiScale)
...
local function createToggleWithKey(parent, yPos, labelText, keybindKey, enabledKey, callback, specialColor)
    ...
    keyBtn.Size = UDim2.new(0, 44 * guiScale, 0, 32 * guiScale)  -- Lebih besar
    keyBtn.Position = UDim2.new(0, 3 * guiScale, 0.5, -16 * guiScale)
    keyBtn.BackgroundColor3 = C.purple
    keyBtn.Text = KEYBINDS[keybindKey].Name
    keyBtn.TextColor3 = Color3.new(1, 1, 1)
    keyBtn.Font = Enum.Font.GothamBold
    keyBtn.TextSize = 14 * guiScale  -- Lebih besar
    keyBtn.ZIndex = 4
    ...
    label.Position = UDim2.new(0, 55 * guiScale, 0, 0)
    label.BackgroundTransparency = 1
    label.Text = labelText
    label.TextColor3 = C.text
    label.Font = Enum.Font.GothamSemibold
    label.TextSize = 16 * guiScale  -- Lebih besar
    ...
    toggleBg.Size = UDim2.new(0, 60 * guiScale, 0, 30 * guiScale)  -- Lebih besar
    toggleBg.Position = UDim2.new(1, -68 * guiScale, 0.5, -15 * guiScale)
    toggleBg.BackgroundColor3 = defaultOn and onColor or Color3.fromRGB(25, 20, 35)
    toggleBg.ZIndex = 4
    Instance.new("UICorner", toggleBg).CornerRadius = UDim.new(1, 0)
    
    local toggleCircle = Instance.new("Frame", toggleBg)
    toggleCircle.Size = UDim2.new(0, 24 * guiScale, 0, 24 * guiScale)  -- Lebih besar
    toggleCircle.Position = defaultOn and UDim2.new(1, -28 * guiScale, 0.5, -12 * guiScale) or UDim2.new(0, 3 * guiScale, 0.5, -12 * guiScale)
    ...
    local function setVisual(state, skipCallback)
        ...
        TweenService:Create(toggleCircle, TweenInfo.new(0.3, Enum.EasingStyle.Back), {Position = isOn and UDim2.new(1, -28 * guiScale, 0.5, -12 * guiScale) or UDim2.new(0, 3 * guiScale, 0.5, -12 * guiScale)}):Play()
...
-- CLEAN SLIDER - TEKS LEBIH BESAR
local function createSlider(parent, yPos, labelText, minVal, maxVal, valueKey, callback)
    local container = Instance.new("Frame", parent)
    container.Size = UDim2.new(1, -10 * guiScale, 0, 64 * guiScale)  -- Lebih tinggi
    ...
    label.Size = UDim2.new(0.6, 0, 0, 24 * guiScale)  -- Lebih besar
    label.Position = UDim2.new(0, 10 * guiScale, 0, 4 * guiScale)
    label.BackgroundTransparency = 1
    label.Text = labelText
    label.TextColor3 = C.textDim
    label.Font = Enum.Font.GothamMedium
    label.TextSize = 14 * guiScale  -- Lebih besar
    label.TextXAlignment = Enum.TextXAlignment.Left
    label.ZIndex = 4
    ...
    valueInput.Size = UDim2.new(0, 60 * guiScale, 0, 28 * guiScale)  -- Lebih besar
    valueInput.Position = UDim2.new(1, -68 * guiScale, 0, 2 * guiScale)
    ...
    valueInput.TextSize = 14 * guiScale  -- Lebih besar
    valueInput.ClearTextOnFocus = false
    valueInput.ZIndex = 4
    Instance.new("UICorner", valueInput).CornerRadius = UDim.new(0, 8 * guiScale)
    
    local sliderBg = Instance.new("Frame", container)
    sliderBg.Size = UDim2.new(0.92, 0, 0, 12 * guiScale)  -- Lebih tebal
    sliderBg.Position = UDim2.new(0.04, 0, 0, 38 * guiScale)
    ...
    thumb.Size = UDim2.new(0, 20 * guiScale, 0, 20 * guiScale)  -- Lebih besar
...
createSlider(leftSide, 60, "Boost Speed", 1, 70, "BoostSpeed", function(v) Values.BoostSpeed = v end)

createToggle(leftSide, 130, "Anti Ragdoll", "AntiRagdoll", function(s)
    Enabled.AntiRagdoll = s
    if s then startAntiRagdoll() else stopAntiRagdoll() end
end)

createToggleWithKey(leftSide, 190, "Spin Bot", "SPIN", "SpinBot", function(s)
    Enabled.SpinBot = s
    if s then startSpinBot() else stopSpinBot() end
end)

createSlider(leftSide, 250, "Spin Speed", 5, 50, "SpinSpeed", function(v) Values.SpinSpeed = v end)

createToggle(leftSide, 320, "Spam Bat", "SpamBat", function(s)
    Enabled.SpamBat = s
...
createToggleWithKey(leftSide, 440, "Bat Aimbot", "BATAIMBOT", "BatAimbot", function(s)
    Enabled.BatAimbot = s
    if s then startBatAimbot() else stopBatAimbot() end
end, C.danger)

createToggle(leftSide, 500, "Galaxy Sky Bright", "GalaxySkyBright", function(s)
    Enabled.GalaxySkyBright = s
    if s then enableGalaxySkyBright() else disableGalaxySkyBright() end
end, Color3.fromRGB(180, 80, 255))

-- Right side toggles - SPACED OUT
...
createSlider(rightSide, 60, "Gravity %", 25, 130, "GalaxyGravityPercent", function(v)
    Values.GalaxyGravityPercent = v
    if galaxyEnabled then adjustGalaxyJump() end
end)

createSlider(rightSide, 130, "Hop Power", 10, 80, "HOP_POWER", function(v) Values.HOP_POWER = v end)

createToggle(rightSide, 200, "Speed While Stealing", "SpeedWhileStealing", function(s)
    Enabled.SpeedWhileStealing = s
    if s then startSpeedWhileStealing() else stopSpeedWhileStealing() end
end)

createSlider(rightSide, 260, "Steal Speed", 10, 35, "StealingSpeedValue", function(v) Values.StealingSpeedValue = v end)

createToggle(rightSide, 330, "Unwalk", "Unwalk", function(s)
    Enabled.Unwalk = s
    if s then startUnwalk() else stopUnwalk() end
end)

createToggle(rightSide, 390, "Optimizer + XRay", "Optimizer", function(s)
    Enabled.Optimizer = s
    if s then enableOptimizer() else disableOptimizer() end
end)

createToggleWithKey(rightSide, 450, "Auto Left", "AUTOLEFT", "AutoWalkEnabled", function(s)
    AutoWalkEnabled = s
...
createToggleWithKey(rightSide, 510, "Auto Right", "AUTORIGHT", "AutoRightEnabled", function(s)
    AutoRightEnabled = s
...
-- Save Button - LEBIH BESAR
local SaveBtn = Instance.new("TextButton", rightSide)
SaveBtn.Size = UDim2.new(1, -10 * guiScale, 0, 60 * guiScale)  -- Lebih besar
SaveBtn.Position = UDim2.new(0, 5 * guiScale, 0, 580 * guiScale)
SaveBtn.BackgroundColor3 = C.purple
SaveBtn.Text = "SAVE CONFIG"
SaveBtn.TextColor3 = Color3.new(1, 1, 1)
SaveBtn.Font = Enum.Font.GothamBold
SaveBtn.TextSize = 18 * guiScale  -- Lebih besar
SaveBtn.ZIndex = 3
Instance.new("UICorner", SaveBtn).CornerRadius = UDim.new(0, 14 * guiScale)
...
infoLabel.Size = UDim2.new(1, 0, 0, 50 * guiScale)  -- Lebih besar
...
infoLabel.TextSize = 11 * guiScale  -- Lebih besar
infoLabel.ZIndex = 3
